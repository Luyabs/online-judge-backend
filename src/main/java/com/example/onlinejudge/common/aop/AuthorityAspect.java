...
import com.example.onlinejudge.common.aop.authoritystrategy.AuthorityStrategy;
import com.example.onlinejudge.common.aop.authoritystrategy.StrategyFactory;
    ...
    public void invoke(JoinPoint joinPoint) {
        ...
        if (null != annotation && (annotation.admin() || annotation.author())) {    // 注解存在，且至少需要管理员或作者一个权限
            ...
            if (annotation.author()) {  // 允许作者访问
                ...
                AuthorityStrategy strategy = strategyFactory.factory(methodName); // 判断作者策略
